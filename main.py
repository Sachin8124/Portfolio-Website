...
from fastapi import BackgroundTasks, FastAPI, Depends, HTTPException, Request, Response, status, UploadFile, File, Form, Query
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import FileResponse, JSONResponse, StreamingResponse
from dotenv import load_dotenv

from database import Session, get_db
from google_drive import download_file as download_drive_file, is_configured as drive_is_configured, upload_file as upload_drive_file
import models
import schemas
...
def send_email_notification(msg: schemas.ContactMessageCreate):
...
{msg.message}
    ...
    email["From"] = f"Portfolio System <{sender_email}>"
    email["Subject"] = f"Portfolio Inquiry: {msg.subject or 'New Message'}"
    email.add_header("reply-to", msg.email)
...
app = FastAPI(
    ...
    description="FastAPI backend with Google Drive storage and scikit-learn NLP auto-categorization",
    version="1.0.0"
)
...
# ==============================================================================
# Google Sheets Initial Seeder (Runs automatically if Sheets is configured)
# ==============================================================================
def seed_initial_data():
    if not drive_is_configured():
        print("Google Drive is not configured; skipping initial portfolio seed.")
        return
    db = next(get_db())
    try:
        ...
        db.rollback()
        print(f"Error seeding Drive storage: {e}")
    finally:
        db.close()
...
def health_check():
    return {
        ...
        "storage": "google_sheets_and_drive",
...
def submit_contact_form(
    ...
    db = next(get_db())
    new_message = models.ContactMessage(**msg_in.model_dump(), status="pending")
    db.add(new_message)
    db.commit()
    db.close()

    background_tasks.add_task(send_email_notification, msg_in)
    return new_message
...
def update_contact_message(
    ...
    message.admin_response = message_update.admin_response
    message.responded_at = datetime.utcnow() if message_update.status != "pending" else None
    db.mark_dirty(models.ContactMessage)
    db.commit()
    db.close()
    return message
...
def upload_file(file: UploadFile = File(...)):
    """Upload portfolio media to Google Drive."""
    original_name = os.path.basename(file.filename or "upload")
    if file.content_type not in ALLOWED_UPLOAD_TYPES:
        raise HTTPException(status_code=415, detail="Only PDF, JPG, PNG, and WebP files are supported.")

    safe_name = f"{uuid.uuid4().hex[:10]}_{original_name.replace(' ', '_')}"

    file.file.seek(0, os.SEEK_END)
    file_size = file.file.tell()
    file.file.seek(0)
    if file_size > MAX_UPLOAD_BYTES:
        raise HTTPException(status_code=413, detail="Certification files must be 10 MB or smaller.")

    if not drive_is_configured():
        raise HTTPException(status_code=503, detail="Google Drive storage is not configured")
    try:
        drive_file_id = upload_drive_file(file.file, safe_name, file.content_type)
    except Exception as error:
        print(f"Google Drive upload failed: {error}")
        raise HTTPException(status_code=503, detail="Media storage is temporarily unavailable") from error
    return {
        "filename": safe_name,
        "url": f"/api/media/{drive_file_id}?type={'pdf' if file.content_type == 'application/pdf' else 'image'}",
...
def serve_drive_media(file_id: str):
    """Proxy a Google Drive file so credentials stay private on the server."""
    try:
        content, media_type = download_drive_file(file_id)
    except Exception as error:
        print(f"Google Drive media download failed: {error}")
